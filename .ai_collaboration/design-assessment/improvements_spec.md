# HWI_HA — Unified Assessment v3: Architecture + Quality Scale + Security

## Purpose

Single authoritative reference merging all assessment work:
- Architecture assessment (system design, data flows)
- Quality scale compliance (Bronze → Gold)
- Security assessment (STRIDE, OWASP IoT Top 10)
- All user `#ANSWER` directives resolved
...
| `binary_sensor.py` (LED) | Yes               | ✅ True          | ❌ No unregister    | **Warn**   | Finding 13 |
...
6. Update `async_send_command()` to use `async_loaded_entries`:
   ...
   for loaded_entry in hass.config_entries.async_loaded_entries(DOMAIN):
       if loaded_entry.runtime_data.controller_id == controller_id:
...
| Option | Pros | Cons |
|--------|------|------|
| **A: Keep as-is** | Simple; self-healing | ~50-200ms overhead per load; noisy logs |
| **B: Guard with version in `entry.data`** | Zero overhead after migration; clean | Requires `async_migrate_entry()` |
| **C: Guard with flag** | Simpler than versions | Doesn't handle future migrations |

**Recommendation**: Option B — move cleanup into `async_migrate_entry()` (Finding 6-legacy). After migration, `async_setup_entry()` reduces to: create coordinator → connect → forward platforms → register listeners.
...
async def async_migrate_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry.version == 1:
        _LOGGER.info("Migrating config entry %s from v1 to v2", entry.entry_id)
...
Set `PARALLEL_UPDATES = 0` in all platform files for explicit documentation. Not a bug — client-layer `asyncio.Lock` already serializes commands. This matches the majority of `CoordinatorEntity`-based integrations in Core.
...
### Finding 9: Broad Exception Handling — **ACCEPTED (skip future enhancements)**
...
| Exception | Source | Catch? | Handling |
|-----------|--------|--------|----------|
| `ValueError` | `CCOAddress.from_string()`, `int()` | ✅ Yes | Log + skip |
| `KeyError` | Missing `CONF_ADDR` | ✅ Yes | Log + skip (corrupt config) |
| `TypeError` | `None` where `str` expected | ✅ Yes | Log + skip |
| `AttributeError` | Code bug (unexpected None) | ❌ No | Let propagate |
| `RuntimeError` | Setup ordering bug | ❌ No | Let propagate |
| `ImportError` | Missing dependency | ❌ No | Let crash |
...
Missing test areas:
- Coordinator KLS→CCO state engine (end-to-end with mocked client)
- Entity lifecycle (`async_added_to_hass` / `async_will_remove_from_hass`)
- Config entry setup and teardown
- Reconnection behavior
- Optimistic update → poll correction flow
- Diagnostics output format
- Config flow connection testing
- CCI button event → state derivation

---

### Finding 12: HACS Manifest Enhancement — **ACCEPTED**

Bump `homeassistant` minimum to `"2024.4.0"`. Add `"content_in_root": false`. Add `"zip_release": true` when release workflow is in place.

Validated: no negative side effects for any of these changes.
...
Demote all `_LOGGER.info("=== ...")` development dumps to `_LOGGER.debug()` across `switch.py`, `config_flow.py`, and `__init__.py`.

---

### Finding 15: Raw Command Injection via `send_command` — **DEFERRED (separate commit)**
...
async def async_send_command(hass: HomeAssistant, data: Mapping[str, Any]) -> None:
    ...
    for command in commands:
        ...
        _LOGGER.info("send_command: controller=%s, command=%s", controller_id, command)
        await client.send_command(command)
```
...
Override `available` in CCO-based entities to detect stale KLS data:
...
Track availability transitions in `_handle_coordinator_update()`:
...
def _handle_coordinator_update(self) -> None:
    ...
    if hasattr(self, "_was_available"):
...
class HomeworksHealthSensor(CoordinatorEntity[HomeworksCoordinator], SensorEntity):
    ...
    _attr_entity_category = EntityCategory.DIAGNOSTIC  # ADD THIS
```
...
| # | Item | Source |
|---|------|--------|
| 17 | Implement `async_migrate_entry()` for legacy format conversion | Finding 6-legacy |
| 18 | Extract shared CCO device creation helper | Finding 5 |
| 19 | Migrate CCO entities to `_attr_has_entity_name = True` | Finding 3 |
| 20 | Reduce `_async_update_data()` to lightweight metadata | Coordinator Issue 2 |
| 21 | Narrow exception handling in entity setup | Finding 9 |
| 22 | Move health sensor icons to `icons.json` | Fix F |

### Phase 4: Observability & Resilience

| # | Item | Source |
|---|------|--------|
| 23 | Add stale-KLS entity unavailability | Fix C |
| 24 | Add availability transition logging | Fix D |
| 25 | Deduplicate `normalize_address` + add sync test | normalize_address |
| 26 | Bump `hacs.json` minimum to `2024.4.0`; add `content_in_root: false` | Finding 12 |
...
## Corrected Findings from Earlier Reviews

| Prior Claim | Actual Status |
|-------------|---------------|
| "No `async_step_reauth()`" | ❌ Wrong — implemented at `config_flow.py` line 1763 |
| "No duplicate entry detection" | ❌ Partially wrong — host:port check exists; missing controller_id check |
| "No timeout in `_try_connection()`" | ❌ Wrong — 10-second `CONNECT_TIMEOUT` in transport |
| "`has_entity_name` conflicted" | ❌ Inaccurate — CCO entities don't set it; inconsistent but no collision |

---

## Deferred Items (separate documents)
